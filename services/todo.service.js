...
function query(filterBy = {}) {
  return storageService.query(TODO_KEY).then((todos) => {
    ...
    if (filterBy.status && filterBy.status !== 'all') {
      todos = todos.filter((todo) => (filterBy.status === 'done' ? todo.isDone : !todo.isDone))
    }
...
function getEmptyTodo(txt = '', importance = 5, color = utilService.getRandomColor()) {
...
function getImportanceStats() {
  return storageService.query(TODO_KEY).then((todos) => {
    ...
    const data = Object.keys(todoCountByImportanceMap).map((level) => ({
      title: level,
      value: todoCountByImportanceMap[level],
    }))
    return data
  })
}
